package com.faroc.gymanager.domain.rooms.events;
...
public record SessionReservationEvent(Room room, Session session) implements DomainEvent {
    ...
    public static String trainerNotFound(UUID trainerId) {
        return "Trainer with id " + trainerId + " not found to add session.";
    }
    ...
    public static String gymNotFound(UUID gymId) {
        return "Trainer with id " + gymId + " not found to add trainer.";
    }
}
